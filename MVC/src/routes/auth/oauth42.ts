...
export async function oauth42(fastify: FastifyInstance) {
  ...
  fastify.get("/42/login", async (req, res) => {
...
fastify.get<{ Querystring: { code?: string } }>("/42/callback", async (req, res) => {
  ...
  try {
    ...
    authService.createSession(req.session, { jwt: authData.jwt, refreshJwt: authData.refreshJwt, user: authData.user });
